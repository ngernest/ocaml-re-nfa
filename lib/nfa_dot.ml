(* Representation of DOT graphs, and conversion from NFAs to DOT
   ...
   The code here supports a slightly restricted subset without subgraphs,
   multi-node edges, or "ports". *)
 ...
 fun nfa ->
  ...
  let make_node =
    ...
    fun n ->
      ...
      let shape =
        if Nfa.StateSet.mem n nfa.Nfa.finals then "doublecircle" else "circle"
      in
      Digraph.Node.with_attrs node [ ("shape", shape) ] in
  let add_edge source c target =
    ...
    | set -> CharSet.add c set in
  let rec step state =
    (* Accumulate nodes and edges, using the states/edges tables as 'seen lists'
       to ensure each node and edge is only visited once *)
  ...
  let dg =
    Digraph.with_node
      ...
      input in
  (** Add the state nodes *)
  let dg = Hashtbl.fold (fun _ node dg -> Digraph.with_node dg node) states dg in

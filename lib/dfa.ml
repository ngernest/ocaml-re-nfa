 ...
 fun f dfa init ->
  ...
  let rec visit state =
    if not (Hashtbl.mem seen state) then (
      ...
      CharMap.iter (fun _ -> visit) (dfa.next state)) in
  visit dfa.start;
  !v
...
let add_transition' (src, c, dst) trans =
  ...
  | cm ->
    let dstset =
      ...
      | dstset -> StateSet.add dst dstset in
  ...
  fun nfa ->
    ...
    let rec build states (map, ts, finals) =
      ...
      | exception Not_found ->
        let state = fresh () in
        let finals =
          if not (StateSet.is_empty (StateSet.inter states nfa.Nfa.finals)) then
        ...
        let map, ts, finals =
          CharMap.fold
            (fun c ss (map, ts, finals) ->
              ...
              (map, ts, finals))
            tsn (map, ts, finals) in
        (state, map, ts, finals) in
    let start, _, trans, finals =
      build nfa.Nfa.start (M.empty, StateMap.empty, StateSet.empty) in

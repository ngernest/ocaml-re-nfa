...
module Letter2Set = struct
  module Pair = struct
    ...
    let compare ((_, w), (_, x)) ((_, y), (_, z)) = Stdlib.compare (w, x) (y, z)
  end
...
(** Datatype for regexes, parameterized over the character type ['c] *)
type 'c regex =
  | Empty : 'c regex (* L = { } *)
  | Eps : 'c regex (* L = {ε} *)
  | Char : 'c -> 'c regex
...
let range_ l h =
  let rec loop i h acc =
    ...
    else loop (succ i) h (C.add (Char.chr i) acc) in
...
module Parse = struct
  ...
  module Bracket = struct
    ...
    let interpret { negated; elements } =
      let s =
        List.fold_right
          ...
          elements C.empty in
      if negated then C.diff any_ s else s
      ...
      match
        C.fold
          (fun c (co, r) ->
            ...
            | c, Some (c1, c2) ->
              (Some (c, c), { r with ranges = (c1, c2) :: r.ranges }))
          set
          ( None,
            { hyphen = false; caret = false; lbracket = false; ranges = [] } )
      with
    ...
    let unparse ?(complement = false) (set : C.t) =
      ...
      (* If we have a single non-special character then there's no need for a
         range expression *)
        when x = y && (not complement) && not (String.contains regex_specials x)
        ->
        pr "%c" x
      | (_ :: _ as rs), lbracket, caret, hyphen ->
        (* If we have some non-special characters then we don't need to take
           extra care to avoid accidentally positioning special characters like
           ^ the beginning or end *)
        bracket
        @@ pr "%s%s%s%s" (whenever lbracket "]") (conc rs) (whenever caret "^")
             (whenever hyphen "-")
      | [], true, _, _ ->
  ...
  (** rseq ::= <empty>
               rsuffixed rseq      *)
  and re_parse_seq : char list -> t * char list =
   fun (s : char list) ->
    match re_parse_suffixed s with
    | None -> (Eps, s)
    | Some (r, rest) ->
      let r', s' = re_parse_seq rest in
      (Seq (r, r'), s')
